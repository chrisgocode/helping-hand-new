...
export function narrateBrowse(effect: BrowseEffect): string {
  switch (effect.kind) {
    case 'categories': {
      ...
      return `You have ${countOf(names.length, 'category')}: ${spokenList(names)}. Say the name of one to hear what is in it.`
    }
    ...
    case 'unknownCategory':
      return `I could not find a category called ${effect.spoken}. Say list my categories to hear them.`
